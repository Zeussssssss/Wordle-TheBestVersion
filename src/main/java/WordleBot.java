...
public class WordleBot {
	...
	public HashMap<Integer, HashMap<String, HashMap<String, String>>> getBotGuessEvaluations() {
		return botGuessEvaluations;
	}

	public String[][] getOptimalGuesses() {
		return optimalGuesses;
	}

	public String[][] getOptimalGuessesEvaluations() {
		return optimalGuessesEvaluations;
	}
	...
	public void evaluate(WordleGame game) {
		...
		for (int i = 0; i < guesses.length; i++) {
			...
			for (String optimalGuess : optimalGuesses.keySet()) {
				...
				optimalGuessData.put("skill",
						String.valueOf(getGuessSkill(optimalGuesses.get(optimalGuess), optimalGuesses, i)));
	...
	private static boolean wordMatchesGuess(String word, String guess, String evaluation) {
		...
		for (int i = 0; i < guess.length(); i++) {
			...
			if ((present.containsKey(guessLetter) || absent.contains(wordLetter)) && guessLetter.equals(wordLetter)
					&& !(correct.containsKey(i))) {
				return false;
			}
